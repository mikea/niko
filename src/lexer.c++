  ...
  *s = YYCURSOR;          \
  ...
  for (;;) {
    ...
    /*!re2c
        ...
        comment { continue; }
        spaces { continue; }
        integer / eow { TOK_VAL(TOK_I64, .val = {.i = str_parse_i64(str(yytext, YYCURSOR))}); }
        float / eow { TOK_VAL(TOK_F64, .val = {.d = str_parse_f64(str(yytext, YYCURSOR))}); }
        "[" / eow { TOK(TOK_ARR_OPEN); }
        "]" / eow { TOK(TOK_ARR_CLOSE); }
        str / eow { TOK_VAL(TOK_STR, .val{.s = str(yytext+1, YYCURSOR-1)}); }
        word"'" / eow { TOK_VAL(TOK_QUOTE, .val{.s = str(yytext, YYCURSOR-1)}); }
